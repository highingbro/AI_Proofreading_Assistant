...
- `run.py`：`run_standard_proofread()`，串起 parse→chunk→build_glossary→proofread→load_learned_feedback→classify全链路，返回 `(ClassifiedResult, ParsedDocument)` 元组（`ParsedDocument` 要传给 `persist_result` 算 `context_snippet`，只在本次调用链上存在，必须一并交出去）。`build_glossary(parsed)`（`core/glossary.py`，补丁：解决chunk间互不可见导致的跨块一致性误判）在分块之后、逐块校对之前跑一次，产出格式化字符串透传给 `proofread_document(..., glossary_text=...)`；`build_glossary` 内部自带异常兜底（失败返回空列表），`run.py` 这里不需要额外 try/except。`load_learned_feedback(db_path)`（`core/feedback.py`，阶段12新增：人工反馈学习自动降级）在逐块校对之后、结果分层之前跑一次，读取历史人工反馈，产出 `list[LearnedFeedback]` 透传给 `classify_issues(..., learned_feedback=...)`，驱动 `core/classifier` 里的规则J（详见 `core/classifier/CLAUDE.md`）；`run_standard_proofread` 因此新增了 `db_path=None` 参数（此前不存在——本函数一直不写库，落库是 `persist_result` 单独负责的，这里只是把 `load_learned_feedback` 需要的库路径从调用方传进来）。
...
`tests/test_stage6.py::test_persist_result_computes_context_snippet_when_parsed_given` 真实构造 `ParsedDocument`+`ParsedBlock` 验证窗口拼接结果。
