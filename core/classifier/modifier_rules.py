...
H(分块边界截断豁免，补丁)，以及优先级覆盖（政治敏感/民族地名）。
...
也不往layer_notes留痕的优先级覆盖）统一签名 (raw, block, tail_blocks, state) -> state，
想知道叠加顺序看 _MODIFIER_RULES 这一个列表就够了，不用通读整个函数体；以后加新
规则只是往列表里插一条并写清楚为什么插在这个位置。详细设计背景见 core/classifier/CLAUDE.md。
"""
...
def _apply_ocr_downgrade(raw: RawIssue, block: ParsedBlock | None, tail_blocks: set[int], state: _ClassificationState) -> _ClassificationState:
...
def _apply_unlocated_cap(raw: RawIssue, block: ParsedBlock | None, tail_blocks: set[int], state: _ClassificationState) -> _ClassificationState:
    if raw.located:
        return state
...
def _apply_recency_downgrade(raw: RawIssue, block: ParsedBlock | None, tail_blocks: set[int], state: _ClassificationState) -> _ClassificationState:
...
def _apply_chunk_boundary_downgrade(raw: RawIssue, block: ParsedBlock | None, tail_blocks: set[int], state: _ClassificationState) -> _ClassificationState:
...
def _apply_priority_override(raw: RawIssue, block: ParsedBlock | None, tail_blocks: set[int], state: _ClassificationState) -> _ClassificationState:
