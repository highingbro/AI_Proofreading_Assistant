...
**②的检测比NFKC能覆盖的范围更广，这是踩过真实数据的坑之后才发现的**：Unicode的"Kangxi Radicals"区块（`U+2F00~2FDF`，如"⽉""⽇""⾓"）大多数字符确实有NFKC兼容分解，直接 `unicodedata.normalize("NFKC", ...)` 就能处理；但同一份真实文档（`data/app.db` 35号记录）里还出现了大量"CJK Radicals Supplement"区块字符（`U+2E80~2EFF`，如"⻔""⻩""⻘""⻓""⻆""⻉""⺟""⺠""⻋""⻨""⻛""⻢"）——这个区块**大多数字符根本没有Unicode兼容分解**，`unicodedata.normalize("NFKC", "⻔")` 原样返回"⻔"，不会变成"门"，NFKC对这类字符完全无效；另外 Kangxi Radicals 区块里的"⼾"虽然**有**分解，但分解目标是繁体"戶"，和简体正文实际需要的"户"不符，同样会被NFKC漏判。因此在NFKC之外单独维护了一张 `postprocess.py::_RADICAL_LOOKALIKE_OVERRIDES` 人工核实过的映射表（13个字符，`_normalize_lookalike()` 先套这张表再做NFKC），**只收录亲自核实过对应关系的字符，不做未验证的猜测性扩充**——错误的映射表会静默篡改比对结果，比漏判更危险，这13个字符全部来自同一份真实文档的实测案例，不是凭空列举Unicode区块里的全部字符。
...
判定（`postprocess.py::_is_visually_no_op_suggestion`，在 `classify_issues` 里于 `classify_issue` **之前**对 `RawIssue` 列表整体过滤，不是叠加在某条已归层结果上的修饰规则）：`_extract_replacement_pair` 抽出"旧→新"文本对后，两者都过 `_normalize_lookalike`（部首替代映射表+NFKC）——归一化后相同即判定为零改动。`_has_stray_control_chars` 独立检查 `original_text` 是否含 `\t\n\r` 之外的Unicode `Cc`类控制字符，命中即无条件丢弃（不依赖suggestion内容——控制字符本身就证明这段"原文"是解析垃圾）。
