...
`tools/compare_columns.py --all` 仍是回归 0 / 四栏收益 36。六条用例（含上述两道闸门各一条
★ 防线）全部做过反向验证。
...
- **接在块级、且必须在 `_order_native_page` 之后**（`_finalize_native_page` 里调）。先按直觉
  ...
  字段才算得出来。`_is_wrapped_continuation` 从 07-28 引入起一直是死代码，就是因为当初接在了
  提取阶段、拿不到分栏信息。
...
**验收改口径了，别再只看全文本 diff**：前几轮解析改动都停在"diff 无回归 + 测试全绿 + 分栏
验收线不变"，三条全绿的同时数据库里的问题数从 54 涨到 108。现在改解析逻辑要跑两样东西：

1. `tools/parse_noise_metrics.py`（零成本、确定性，输出 句中换行/位置断行/括号错位 三个数，
   **只准改动针对的那个降、另外两个不许变坏**）。实测 句中换行 9467→4810、位置断行 6050→0、
   括号错位 15→15 不变；`tools/compare_columns.py --all` 仍是回归 0 / 四栏收益 36。
...
**只做了块级、还没做块内的中途曾出现"N3短 8→45、总数不降"**，当时逐条核过那 45 条全落在一页
活动日历上、且那一页的块文本改动前后逐字节相同——**不是回归，是块内那一半还没做**。这里记下来
是因为它示范了正确的判读方式：看分类、并且核对变化的那一栏对应的块文本到底变没变，别看总条数
（同一份文档连跑五次总数在 35/71/83/54 之间波动）。**"修伪影不会让总条数下降"是个错误结论**，
两边都做完之后总条数确实降了。
...
**`sample.pdf` 和武昌首义手册这次不再是"diff 必须为空"**——它们虽是单栏 Word→PDF，PyMuPDF
同样把段落拆成了逐行 block（sample.pdf 37→23 块），diff 逐行核过全部是正确的段落还原。十四条
