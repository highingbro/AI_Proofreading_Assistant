...
- 子栏间距用单独的、更低的阈值 `config.COLUMN_SUB_GAP_MIN_WIDTH_RATIO`(0.03)。实测该刊四栏页主栏间距占整页7.5%，子栏间距只占半区内容宽度3.8%，沿用主栏阈值(0.05)会整片漏检。半区宽度按该半区**实际内容范围**算（`_content_extent`），不是按分栏线切出来的几何半幅——后者会把页边距算进去，band(中部40%~60%)的落点和宽度占比都会偏。
...
验证：修复后重跑上述两份期刊，两处假错误的成因文本都恢复成连贯句子；单栏文档（`系统管理员.pdf` 34页、`武昌首义学院…学生版` 38页、`sample.pdf` 4页）全部仍判为单栏，没有因为放宽阈值产生新的误判。测试见 `tests/test_parser.py`"分栏检测的两处真实误判"一节五条用例。
