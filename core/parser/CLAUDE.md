...
分栏检测结果不变（`tools/compare_columns.py --all` 仍是回归 0 / 四栏收益 36）。四条用例都做过
反向验证（把删除逻辑退成恒等函数，确认真会红）。

**同一批诊断里还发现三类未处理的文本装配伪影**，都已实测到字符坐标级别，按噪声量排序：
全角开括号墨迹在字框右半导致 `1.《书名》`被拼成 `1《 . 书名》`（96 处）；排版字距微调被编码
成真空格导致 `APS`→`A PS`、`Autodesk`→`A u t o d e s k`（113 处）；目录行"标题……页码"被拼成
`标题\n页码`（2 处，两条候选修法都会误伤真实表格数据，暂不处理）。
