...
- **字符数统计量**（旧算法的两侧字符数平衡、非通栏块字符占比）：随页面内容剧烈波动，
...
结构上（主缝 + 两半各自细分 + 要求两半都成功才算四栏）与被替换的旧递归二分相同——
递归本身没错，错的是每一层用什么信号判断"这里是不是空白"。

## 安全方向：宁可判少，绝不凭空补线

缺一条真缝时降级到栏数更少的假设，**不做"按栏距外推补齐缺失边界"**。判少了最坏是维持
今天的交错行为；凭空补一条不存在的分栏线会把正文切碎，是唯一能比旧算法更糟的失败模式。
...
工作点（134 页目视核定 GT 命中 129、相对旧算法收益 41、回归 0，另 221 页单栏零误判）
与每个常量的两侧余量见 `tools/column_probe_data.md`。**新文档出分栏问题时第一步是跑
`tools/probe_columns.py` 看占用率分布，而不是直接调阈值**——这几个阈值标定自同一家
排版的 3 份期刊，τ 与 `COLUMN_A_SPANNING_WIDTH_RATIO` 的上界还是由同一页（唯一的整页
大表格样本）决定的，过拟合风险最高的就是这两个。
"""
