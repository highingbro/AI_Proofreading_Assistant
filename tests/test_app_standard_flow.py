...
def test_run_standard_proofread_calls_pipeline_in_order():
    ...
    mock_classify.assert_called_once_with(
        fake_proofread_result, fake_parsed, fake_chunked, mode=config.PROOFREAD_MODE_DEEP
    )
    # 返回 (ClassifiedResult, ParsedDocument) 元组：parsed 要传给 persist_result
...
def test_persist_result_writes_record_and_issues_to_temp_db(db_path):
    ...
    # 传 parsed 时的填充行为见 test_followup.py::test_persist_result_computes_context_snippet_when_parsed_given。
