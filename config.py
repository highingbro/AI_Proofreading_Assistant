...
# 已判出主分栏线后，在左右两半区域内再找一层子栏间距时的最小宽度比例（相对该半区
# 自身的内容宽度，不是整页宽度）。四栏版面的栏间距比两栏版面窄得多——真实数据：
# 某期刊四栏页主栏间距占整页7.5%，子栏间距只占半区内容宽度3.8%，沿用
# COLUMN_MIN_GAP_WIDTH_RATIO(0.05) 会整片漏检，故单列一个更低的阈值；放低阈值
# 带来的误判风险由两侧字符数平衡校验、以及"左右两半必须都能再分"的对称性要求兜底。
COLUMN_SUB_GAP_MIN_WIDTH_RATIO = 0.03
