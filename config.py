...
# 每批最多这么多个块并发。**卡脖子的是模型服务端的实际并发处理能力，不是账号限流**
# （账号实测 RPM 15000/TPM 1200000，远够用）：data/app.log 显示 29 个块一次性全发时，
# 先完成的也要170~230s（低并发下同一天是47~66s），后完成的拖到300~570s甚至撞上超时
# 上限集体失败，而失败后的整批重试会把同样的拥堵原样重演一次。过了拥堵拐点再加并发
# 边际收益为负，总耗时反而比不设上限更短。8 凭日志间接推断（不是精确压测得出）。
...
# MKL-DNN+PIR 路径（见 core/parser/ocr_pdf.py 里对设备与 MKL-DNN 的处理）。
PADDLE_DEVICE = "auto"
...
# 工作点：134页目视核定GT命中129，另221页单栏零误判。每个值两侧的余量见下方各条注释，
# **改之前逐条读**——多个常量的安全窗只有一两档宽，"看着还能再放一点"通常是错的。
# ---------------------------------------------------------------------------
...
# 0.06 是收益曲线的膝点（0.05→命中126、0.06→129、0.075 仍129、0.08 起出错），
...
# A类：PDF字体的ToUnicode表把某个字形映射成**另一个毫不相干、看着完全正常的汉字**时，
# 靠渲染该行送OCR来读出真身（检测与还原都在 core/parser/_glyph_repair.py）。总开关：
# 关掉则只检测、不还原，全部按读不出来处理（纯CPU机器嫌慢可关，代价是盲区变大）。
NATIVE_GLYPH_REPAIR = True
# 同上：只有当该行**非伪造位置**上 OCR 与文字层的一致率达到此值，才采纳 OCR 在伪造位置
# 给出的字——这行别处都读不准，伪造位取它就不可信。真实文档实测一致率分布是
# {0.5: 3处, 0.9: 30处, 1.0: 62处}，取0.8保留92/98；剩下那3处来自"• 数百微信大群"这种
# 极短行（可比对的字本来就没几个，其实还原对了），按"宁可丢弃不出错"接受误杀。
NATIVE_GLYPH_REPAIR_MIN_LINE_AGREEMENT = 0.8
# 同上：渲染待识别行时的分辨率。比整页OCR的 OCR_RENDER_DPI 高，因为这里只渲染一行、
# 成本可以忽略，而单字识别准确率对分辨率敏感。
NATIVE_GLYPH_REPAIR_DPI = 400
# 同上：还原不了的字符在正文里留下的记号。**这不是给人看的标记，是给程序定位用的**——
# 要让 core/chunker/ 丢掉"含读不出字符的那一句"，就得知道它落在块文本的哪个位置，而块文本
# 要经段落合并与字形归一化才成形，跟踪字符偏移量既啰嗦又脆；留一个记号在文本里，它自己
# 就是那个位置标记。取全角空心方框：中文校勘里它本就是"此处一字残缺待补"的通行记法，
# 且不可能作为正文内容出现。含它的整句不送LLM，因此它不会出现在问题卡或导出表里。
NATIVE_UNREADABLE_GLYPH_MARK = "□"
