...
- 用户名隔离（简单版）：按用户名分开保存各自的校对记录/上传文档，不做密码校验，适合几个互相信任的人共用同一份部署；不填用户名默认沿用同一份数据
- Excel 导出
...
# 启动应用前需设置环境变量 DASHSCOPE_API_KEY（必需，DashScope密钥）
# LLM_BASE_URL / LLM_MODEL 可选，不设置则使用默认值
streamlit run app.py
```

多人共用同一份部署时，建议每个人在侧边栏"当前用户"里填自己的名字——各自的校对记录和上传文档会分开保存，互不覆盖。这不是登录鉴权，只是按名字分文件，不要用来防止互相查看。
...
这些都是本地文件，没有自动备份机制，迁移/更换设备前请自行复制 `data/` 目录。
